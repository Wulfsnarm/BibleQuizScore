            ...
            for (book in data[set]){
                for (chapter in data[set][book]){
                    for (verse in data[set][book][chapter]){
                        result_string = result_string + "<b>" + book + " " + chapter + ":" + verse + "</b> " + data[set][book][chapter][verse] + "\n\n";
                ...
                for (book in data[set]){
                    for (chapter in data[set][book]){
                        for (verse in data[set][book][chapter]){
                            ...
                            var txt = data[set][book][chapter][verse];
                            var match = true;
                            ...
                            if (!(search_terms.length == 1 && search_terms[0] == "")){
                                ...
                                if (match)	{		
                                    ...
                                    if (!book_match && !chap_match && !vers_match){
                                        pre_n_post = extra_verses(extra, data, set, book, chapter, verse, result_string);
...
function extra_verses(count, data, set, book, chap, vers, result_string){
    pre_result = "";
    post_result = "";
    verse_list = Object.keys(data[set][book][chap])
    if (count > 0){
        for (ext_i = 1; ext_i <= count; ext_i++){
            if (verse_list.indexOf(vers)-ext_i >= 0){
                pre_verse = verse_list[verse_list.indexOf(vers)-ext_i]
                pre_result = "<b>" + book + " " + chap + ":" + pre_verse + "</b> " + data[set][book][chap][pre_verse] + "\n" + pre_result;
            }
            if (verse_list.indexOf(vers)+ext_i < verse_list.length){
                post_verse = verse_list[verse_list.indexOf(vers)+ext_i]
                post_result = post_result + "<b>" + book + " " + chap + ":" + post_verse + "</b> " + data[set][book][chap][post_verse] + "\n";
            }
        }
    }
...
function sort_data(){
    let sort = data["sort"].split(",");
    let sorted_data = [];

    for (let i = 0; i < sort.length; i++) {
        let parts = sort[i].trim().split(" ");
        let book = parts[0];
        let chapter = parts[1];
        let verse = parts[2];
        ...
        if (chapter.includes("-")) chapter = chapter.split("-");
        if (verse && verse.includes("-")) verse = verse.split("-");

        // Each `sort[i]` gets its own this_set
        let this_set = {};

        // Make sure we're working with strings for consistency
        let chapStr = Array.isArray(chapter) ? chapter[0].toString() : chapter.toString();
        if (!this_set[book]) this_set[book] = {};
        if (!this_set[book][chapStr]) this_set[book][chapStr] = {};

        if (Array.isArray(verse)) {
            let [start_vers, end_vers] = verse.map(v => parseInt(v));
            for (let x = start_vers; x <= end_vers; x++) {
                if (data[book]?.[chapStr]?.[x]) {
                    this_set[book][chapStr][x] = data[book][chapStr][x];
                }
            }
        } else if (verse) {
            if (data[book]?.[chapStr]?.[verse]) {
                this_set[book][chapStr][verse] = data[book][chapStr][verse];
            }
        }

        sorted_data.push(this_set);
    }
